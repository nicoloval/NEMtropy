...
jit(forceobj=True)
def random_weighted_matrix_generator_uniform_custom_density(n, p=0.1 ,sup_ext = 10, sym=False, seed=None, intweights = False):
    if sym==False:
        np.random.seed(seed = seed)
        A = np.zeros(shape=(n, n))
        for i in range(n):
...
jit(forceobj=True)
def random_weighted_matrix_generator_gaussian_custom_density(n, mean, sigma, p=0.1, sym=False, seed=None, intweights = False):
    if sym==False:
        np.random.seed(seed = seed)
        A = np.zeros(shape=(n, n))
        for i in range(n):
        ...
        for ind, ki, ko in zip(np.arange(k_in.shape[0]),k_in,k_out):
            if (ki==0) and (ko==0):
                while((np.sum(A[:,ind])==0) and (np.sum(A[ind,:])==0)):
                    if np.random.random() >0.5:
                        A[np.random.randint(A.shape[0]), ind] = np.random.normal(loc=mean, scale=sigma)
                    else:
                        A[ind,np.random.randint(A.shape[0])] = np.random.normal(loc=mean, scale=sigma)
        ...
        for ind, k in enumerate(degree):
            if k==0:
                while(np.sum(A[:,ind])==0):
                    ...
                    if indices!= ind:
                        aux_ind = np.random.randint(A.shape[0])
                        A[aux_ind, indices] = np.random.normal(loc=mean, scale=sigma)
                        A[indices, aux_ind] = A[aux_ind, indices]
        if intweights:
            return np.ceil(A)
        else: 
